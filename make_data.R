...
get_session1<- function(data){
  ...
  if ("condition" %in% colnames(data)){
    ...
    trials$ISI<-as.numeric(trial_data[trial_data$screen == "fixation" & !is.na(trial_data$screen), "trial_duration"])
    trials$Session <- 1
    trials$Fixation<- trial_data[trial_data$screen=='fixation', 'stimulus']
  }
  else
  {
    print(paste0("Check Session Info for", file))
  }
  trials
}
...
session1_scales<- function(file){
  ...
  if("IPIP6" %in% data$screen) { 
    ...
    df$AttentionCheck_Score1<- ipip6$Attention_Check_1
    df$AttentionCheck_Score2<- ifelse(pid5$Attention_Check_2==3,1,0)
    df$AttentionCheck_Score3<- ifelse(spq$Attention_Check_3==4,1,0)
    df$AttentionCheck_Score4<- ifelse(asq28$Attention_Check_4==0,1,0)
    df$AttentionCheck_Score5<- ias$Attention_Check_5
    df$AttentionCheck_Score6<- ifelse(gcbs$Attention_Check_6==0,1,0)
    df$AttentionCheck_Score7<- lie$Attention_Check_7
  } 
  else 
  {
...
session2_scales<- function(file){
  ...
  if("MAIA" %in% data$screen) { 
    ...
    df$AttentionCheck8_Score <- maia$Attention_Check_1
    df$AttentionCheck9_Score <- bpd$Attention_Check_2
    df$AttentionCheck10_Score <- ifelse(pi18$Attention_Check_3==5, 1, 0) 
  }
  df
}
...
preprocess_raw <- function(file) {
  ...
  temp <- gsub(";", '>', (gsub("<p style=\"color: black; font-size: 80px; ", "", trials$Fixation)))
