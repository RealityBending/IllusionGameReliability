...
plot_desc_errors <- function(data) {
  ...
  dat |>  
    ...
    geom_bar(data = dat |> 
               ...
               summarize(Illusion_Strength = mean(Illusion_Strength),
                               Error = sum(Error) / n()),
                     aes(y=Error, fill = Illusion_Difference),
                   alpha = 1/3, position = "dodge", stat = "identity", width = diff(range(dat$Illusion_Strength)) / 10) +
