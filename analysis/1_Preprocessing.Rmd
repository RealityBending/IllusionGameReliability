...
sub <- read.csv("../data/raw_questionnaires.csv") |> 
  mutate(
    Education = fct_relevel(Education, "Other", "High school", "Bachelor", "Master", "Doctorate", "Prefer not to Say")
  )
...
#   plot_distribution()
p3 <- data |> 
  filter((as.numeric(gsub("\\D", "", Participant)) >= 300) | (as.numeric(gsub("\\D", "", Participant)) <= 2)) |>
  plot_distribution() 
# p3
...
outliers_questionnaires <- c(
  "S003", "S008", "S068", "S110", "S127", "S151", "S189", "S266"
)
```
